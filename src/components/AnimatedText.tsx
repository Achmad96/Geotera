...
const AnimatedText = memo(
  ({
    ...
    useEffect(() => {
      if (isInView) {
        controls.start("visible").then(() => console.log("visible"));
      } else {
        setIsAnimationComplete(false);
        controls.start("hidden").then(() => console.log("hidden"));
      }
    }, [isInView, controls]);
        ...
        >
          {text.split(" ").map((word, wordIndex) => {
            if (underlineWords && wordIndex !== underlineWords[0]) {
              if (
                wordIndex > underlineWords[0] &&
                wordIndex <= underlineWords[1]
              ) {
                delay += 0.3;
              }
            }
